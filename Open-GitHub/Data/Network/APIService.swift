...
class APIService: APIServiceDelegate, APIServiceFormatter {
    ...
    public func getRepositories(query: String, sortBy: String, completion: @escaping (GitHubSearchRepositoriesResponse) -> Void) {
        let dict = ["q": query, "sort": sortBy]
        let url = getUrlString(dict, api: .main, endPoint: .searchRepository)
